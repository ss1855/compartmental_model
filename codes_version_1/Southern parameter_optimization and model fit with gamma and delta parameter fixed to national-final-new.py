...
def main(df):
    ...
    x0 = np.array([(319502247 - 8 - 28 - 3 - 1), 8, 28, 3, 1, 32, 3, 1])
...
if __name__ == "__main__":
    df = read_data("SouthernCOVID-rolling-avereage.xlsx")
    par = main(df)
...
ax1.set_title('I',fontsize =14,loc ="right")
ax2.set_title('J',fontsize =14,loc ="right")

ax1.text(0.5, 0.9, 'Southern zone', horizontalalignment='center', verticalalignment='center', transform=ax1.transAxes)
...
fig.tight_layout()
fig.savefig(os.path.join('Output','Southern_fitfinal_new.png'),dpi = 300)
plt.close()
...
pd.DataFrame(par).to_excel("southern_parameter.xlsx")
