...
def main(df):
    ...
    x0 = np.array([(181865713 - 5 - 28 - 0 - 0), 5, 28, 0, 0, 28, 0, 0])
...
if __name__ == "__main__":
    df = read_data("NorthernCOVID-rolling-avereage.xlsx")
    par = main(df)
...
ax1.text(0.5, 0.9, 'Northern zone', horizontalalignment='center', verticalalignment='center', transform=ax1.transAxes)
...
fig.tight_layout()
fig.savefig(os.path.join('Output','Northern_fitfinal_new.png'),dpi = 300)
plt.show()
plt.close()
...
pd.DataFrame(par).to_excel("Northern_parameter.xlsx")
