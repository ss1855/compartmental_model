...
def main(df):
    ...
    x0 = np.array([(53144769 - 5 - 1 - 0 - 0), 5, 1, 0, 0, 1, 0, 0])
    tf = [37, 51, 79, 140, 191, 201, 215, 227, 253, 293]
    t0 = 11
    delta1 = []
    gamma1 = []
    lambd1 = []
    ...
    b = np.array(beta1).reshape(1, 10)
    c = np.array(gamma1).reshape(1, 10)
    d = np.array(delta1).reshape(1, 10)
    e = np.array(lambd1).reshape(1, 10)
    par = np.concatenate([b, c, d, e], axis=0).T
    return par
if __name__ == "__main__":
    df = read_data("MostCOVID-rolling-avereage.xlsx")
    par = main(df)
...
def params(t):
    ...
    params = {'betasymptomatic': par[9, 0], 'gamma': par[9, 1], 'delta': par[9, 2], 'lambd': par[9, 3]}
    return params
...
def latentsamplehypercube(n):
    ...
    return np.array(c).flatten().reshape(10,4*n)
...
ax1.set_title('K',fontsize =14,loc ="right")
ax2.set_title('L',fontsize =14,loc ="right")

ax1.text(0.5, 0.9, 'NorthEastern zone', horizontalalignment='center', verticalalignment='center', transform=ax1.transAxes)
...
ax1.fill_between(df_time["Time"][11:293],low_percentile_infected, upper_percentile_infected,color ="lightgrey")
ax1.fill_between(df_time["Time"][11:293], q1_values_infected, q3_values_infected, color="lightcoral")
ax1.plot(df_time["Time"][11:293],avg_T[5],linestyle="--",color ="black", label ="Average", alpha = None)
ax1.plot(df_time["Time"][11:293],tesko[5],color ="black", label ="Model fit",alpha = None)
ax1.scatter(df_time["Time"][11:293][::5],df["Cinfected"][11:293][::5],marker = "o",facecolors="None", label = "Reported",edgecolor = "blue")
...
ax2.fill_between(df_time["Time"][11:293],low_percentile_deceased, upper_percentile_deceased,color = "lightgrey")
ax2.fill_between(df_time["Time"][11:293], q1_values_deceased, q3_values_deceased, color="lightcoral")

#ax.plot(df["Time"],avg_T[6],linestyle="--",color ="black", label ="average")
ax1.ticklabel_format(axis = 'y',style ='sci',scilimits =(0,0))
ax2.plot(df_time["Time"][11:293],avg_T[7],linestyle="--",color ="black", label ="Average",alpha = None)
ax2.plot(df_time["Time"][11:293],tesko[7],color ="black", label ="Model fit",alpha = None)
ax2.scatter(df_time["Time"][11:293][::5],df["Cdeath"][11:293][::5],label = "Reported", marker = "o",facecolors="None",edgecolor = "blue")
...
fig.tight_layout()
fig.savefig(os.path.join('Output','NorthEastern_fitfinal.png'),dpi = 300)
plt.close()
