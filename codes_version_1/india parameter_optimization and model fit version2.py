...
def set_parameters():
    ...
    params.add("lambd", value=lambd, vary=True, min=1/5, max=1/2)
...
fig.tight_layout()
fig.savefig(os.path.join('Output','Cnew.png'),dpi = 300)
plt.show()
plt.close()
...
for a in range(11):
    for i in range(1000):
        r = parameter_sample[a,i]/(parameter_sample[a,i+1000]+parameter_sample[a,i+2000])
        s.append(r)
...
percentile_high = []
for i in range(11):
    a = np.percentile(s2[i],25)
    b = np.percentile(s2[i],75)
    percentile_low.append(a)
    percentile_high.append(b)
...
plt.bar(range(len(ratio)), ratio, color='lightgrey')
plt.xlabel('Sub-Periods in 2020')
