...
def set_parameters(t):
    beta = 0.20
    delta = (1/60)/365
    gamma_ = [0.03333333, 0.2       , 0.04728109, 0.2       , 0.2       ,
       0.2       , 0.03333333, 0.2       , 0.2       , 0.17079032,
       0.2       ]
    lambd_= [0.2       , 0.5       , 0.36962981, 0.2113175 , 0.43798761,
       0.5       , 0.2       , 0.31800263, 0.5       , 0.5       ,
       0.5       ]
    thresholds = [(11, 0),
        (37, 1),
        (51, 2),
...
def main(df):
    ...
    x0 = np.array([(316596628 - 5 - 1 - 0 - 0), 5, 1, 0, 0, 1, 0, 0])
...
if __name__ == "__main__":
    df = read_data("EasternCOVID-rolling-avereage.xlsx")
    par = main(df)
...
ax1.set_title('E',fontsize =14,loc ="right")
ax2.set_title('F',fontsize =14,loc ="right")

ax1.text(0.5, 0.9, 'Eastern zone', horizontalalignment='center', verticalalignment='center', transform=ax1.transAxes)
...
ax1.fill_between(df_time["Time"][2:293],low_percentile_infected, upper_percentile_infected,color ="lightgrey")
ax1.fill_between(df_time["Time"][2:293], q1_values_infected, q3_values_infected, color="lightcoral")
ax1.plot(df_time["Time"][2:293],avg_T[5],linestyle="--",color ="black", label ="Average", alpha = None)
ax1.plot(df_time["Time"][2:293],tesko[5],color ="black", label ="Model fit",alpha = None)
ax1.scatter(df_time["Time"][2:293][::5],df["Cinfected"][2:293][::5],marker = "o",facecolors="None", label = "Reported",edgecolor = "blue")
...
ax2.fill_between(df_time["Time"][2:293],low_percentile_deceased, upper_percentile_deceased,color = "lightgrey")
ax2.fill_between(df_time["Time"][2:293], q1_values_deceased, q3_values_deceased, color="lightcoral")

#ax.plot(df["Time"],avg_T[6],linestyle="--",color ="black", label ="average")

ax2.plot(df_time["Time"][2:293],avg_T[7],linestyle="--",color ="black", label ="Average",alpha = None)
ax2.plot(df_time["Time"][2:293],tesko[7],color ="black", label ="Model fit",alpha = None)
ax2.scatter(df_time["Time"][2:293][::5],df["Cdeath"][2:293][::5],label = "Reported", marker = "o",facecolors="None",edgecolor = "blue")
...
fig.tight_layout()
fig.savefig(os.path.join('Output','Eastern_fitfinal-new.png'),dpi = 300)
plt.show()
plt.close()
...
pd.DataFrame(par).to_excel("eastern_parameter.xlsx")
