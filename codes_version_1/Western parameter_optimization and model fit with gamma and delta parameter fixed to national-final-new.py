...
def main(df):
    ...
    x0 = np.array([(196826216 - 15 - 26 - 0 - 0), 15, 26, 0, 0, 26, 0, 0])
...
if __name__ == "__main__":
    df = read_data("WesternCOVID-rolling-avereage.xlsx")
    par = main(df)
...
ax1.set_title('G',fontsize =14,loc ="right")
ax2.set_title('H',fontsize =14,loc ="right")

ax1.text(0.5, 0.9, 'Western zone', horizontalalignment='center', verticalalignment='center', transform=ax1.transAxes)
...
fig.tight_layout()
fig.savefig(os.path.join('Output','Western_fitfinal_new.png'),dpi = 300)
plt.show()
plt.close()
...
pd.DataFrame(par).to_excel("western_parameter.xlsx")
